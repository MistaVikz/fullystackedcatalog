...
Functionality: (in application.py)
	...
	3. createUser(), getUserInfo(), getUserId():
		Uses the login session to create a new entry in the User table. Return user information for a specific user.
	4. catalog(), catView(), itemNew(), itemEdit(), itemDelete():
		Provides routing to all pages in the CAT-A-LOG. Ensures that only logged in users can Create, Edit or Delete 
		items. Also ensures that users can only edit and delete items that they have created.
