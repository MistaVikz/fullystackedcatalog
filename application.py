...
def gconnect():
    ...
    if result['user_id'] != gplus_id:
        response = make_response(
                json.dumps("Token user ID doesn't match current user ID."), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def itemEdit(category_id, item_id):
    ...
    if editItem.user_id != login_session['user_id']:
        flash(login_session['email'] + ". You are not authorized to edit this item.")
        return redirect(url_for('catView', category_id=category_id))
...
def itemDelete(category_id, item_id):
    ...
    if deleteItem.user_id != login_session['user_id']:
        flash(login_session['email'] + ". You are not authorized to delete this item.")
        return redirect(url_for('catView', category_id=category_id))
    if request.method == 'POST':
...
catSecret = ''.join(random.choice(string.ascii_uppercase 
        + string.digits) for x in xrange(32))
